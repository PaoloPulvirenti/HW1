...
{
    {
        fetch("Spotify.php?em="+encodeURIComponent(String(event.currentTarget.value))+"&t=Playlist").then(onResponse).then(onJson);
...
{   
    ...
    fetch("Spotify.php?em="+encodeURIComponent(String(id))+"&t=Musica").then(onResponse).then(onJsonPlaylist);
}
